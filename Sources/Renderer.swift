...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func setupScene() {
        ...
        for i in 0..<15 {
            ...
            let size = Float.random(in: 1.2...2.0)
            
           // Compute accurate bounding sphere centered on the quad (offset in local -Z)
           let rotationMatrix = matrix4x4_rotation(radians: rotY, axis: [0, 1, 0])
           let localOffset = SIMD3<Float>(0, 0, -1.8)  // Quad center in local space
           let scaledOffset = localOffset * size
           let rotatedOffset4 = rotationMatrix * SIMD4<Float>(scaledOffset.x, scaledOffset.y, scaledOffset.z, 0)
           let rotatedOffset = SIMD3<Float>(rotatedOffset4.x, rotatedOffset4.y, rotatedOffset4.z)
           let sphereCenter = SIMD3<Float>(x, y, z) + rotatedOffset
           let sphereRadius = size * 1.1  // Tight radius with slight padding for easier hover
           ...
           let frustum = CameraFrustum(
               ...
               size: size,
               photoTexture: nil,
               boundingSphere: BoundingSphere(center: sphereCenter, radius: sphereRadius)
           )
            
            cameraFrustums.append(frustum)
            print("📦 Created frustum \(i): position=\(frustum.position), sphere center=\(frustum.boundingSphere.center), radius=\(frustum.boundingSphere.radius)")
        }
    }
    ...
    func draw(in view: MTKView) {
        ...
        for (index, frustum) in cameraFrustums.enumerated() {
            if let v = viewportFrustumIndex, index == v && !isFlyingTo {
                       continue  // Hide current frustum when arrived (Photosynth-like immersion)
                   }
            ...
            let alpha: Float = frustum.isHovered ? 0.6 : 0.3  // Higher opacity on hover
            
            var uniformsWithAlpha = UniformsWithAlpha(
                modelMatrix: modelMatrix, 
                viewMatrix: viewMatrix, 
                projectionMatrix: projectionMatrix,
                alpha: alpha // 30% opacity for transparency
            )
...
//        camera.rotation = SIMD3<Float>(0, 0, 0) // Look straight ahead
    ...
    func handleMouseMove(at location: CGPoint, viewSize: CGSize) {
        ...
        print("🖱️ Mouse moved to: \(location) in view size: \(viewSize)")
           ...
           let ndcY = (2 * Float(location.y) / Float(viewSize.height)) - 1  // Correct: bottom -1, top +1
            
        
       if ndcX < -1 || ndcX > 1 || ndcY < -1 || ndcY > 1 {
           print("⚠️ NDC outside frustum (-1 to 1): (\(ndcX), \(ndcY)) - mouse likely off-screen")
       }
        ...
        print("🔦 Ray direction.z: \(ray.direction.z) (should be negative for forward in world space)")
        ...
        for (index, frustum) in cameraFrustums.enumerated() {
            ...
            let screenY_flipped = (1 - ndcPos.y) * Float(viewSize.height) / 2
             ...
             let projected_point = CGPoint(x: CGFloat(screenX), y: CGFloat(screenY_flipped))  // Use flipped if matches screen y=0 bottom
             let mouse_dist = hypot(Float(location.x - projected_point.x), Float(location.y - projected_point.y))
             print("   📏 Pixel dist mouse to projected center: \(mouse_dist) (small ~ hover if radius projects to ~50px)")
            ...
            if let t = intersectRaySphere(ray: ray, sphere: sphere) {
             ...
             var min_distance_to_center: Float = .infinity
             var closest_frustum_index: Int = -1
             for (index, frustum) in cameraFrustums.enumerated() {
                 let dist = length(frustum.boundingSphere.center - ray.origin)
                 if dist < min_distance_to_center {
                     min_distance_to_center = dist
                     closest_frustum_index = index
                 }
             }
             print("📏 Closest sphere (even if miss): index \(closest_frustum_index), dist \(min_distance_to_center)")
        }
    ...
    func handleClick(at location: CGPoint, viewSize: CGSize) {
        // If already hovering a frustum, directly fly to it (faster, no ray cast needed)
        if let hoveredIndex = hoveredFrustumIndex {
            print("🎯 Click on hovered frustum \(hoveredIndex) - starting direct zoom")
            flyToFrustum(at: hoveredIndex)
            return
        }
        
        // Fallback: Full ray cast if no hover (e.g., click without prior mouse move)
        print("🖱️ No hover - performing full ray cast for click")
        ...
        let invProjMatrix = projMatrix.inverse
        var eyeCoords = invProjMatrix * clipCoords
        eyeCoords.z = 1  // Forward direction
        eyeCoords.w = 0
        ...
        if let frustum = closestFrustum {
            ...
            if let frustumIndex = cameraFrustums.firstIndex(where: { $0.position == frustum.position }) {
                ...
                flyToFrustum(at: frustumIndex)  // Use existing fly method
            }
        }
    }
    ...
    func intersectRaySphere(ray: Ray, sphere: BoundingSphere) -> Float? {
        print("🔍 Debug for sphere center=\(sphere.center), radius=\(sphere.radius)")
        let oc = ray.origin - sphere.center
        
        let to_center_dir = normalize(sphere.center - ray.origin)
        let cos_angle = dot(ray.direction, to_center_dir)
        print("   cos_angle to center: \(cos_angle) (positive means pointing toward sphere, likely hit possible)")
        print("   oc=\(oc), length(oc)=\(length(oc))")
        ...
        if discriminant < 0 {
            print("   ❌ Miss: negative discriminant (ray pointing away or too far?)")
            return nil
        }
        ...
        print("   ❌ Miss: negative t values (intersection behind ray origin)")
        return nil
